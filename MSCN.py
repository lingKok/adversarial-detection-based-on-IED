...
def local_deviation(image, local_mean, kernel):  ##计算标准差有点和原文不太一样
    # sigma=(image-local_mean)**2
    # return signal.convolve2d(sigma,kernel,'same')
    sigma = image ** 2
...
def asymmetric_generalized_gaussian_fit(x):
    ...
    def estimate_r_hat(x):
        ...
        print(x.shape)
        out=(np.sum(np.abs(x)) / size) ** 2 / (np.sum(x ** 2) / size)
        return out
