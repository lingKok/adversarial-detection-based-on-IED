...
def tsne(X,Y,args):
    ...
    norm = X_tsne[index_0]
    adv = X_tsne[index_1]
    plt.scatter(norm[:,0],norm[:,1],c='darkviolet',s=6,label='normal example')
    plt.scatter(adv[:, 0], adv[:, 1], c='lime', s=6, label='adversarial example')
    plt.rcParams.update({'font.size':15})
    plt.legend(loc='upper right')
    plt.title(args.dataset,fontsize=25)
    plt.show()
    return X_tsne,Y
...
def main():
    ...
    test_feature_path = './detect_train/layers_features/{}_layers_test.npy'.format(dataset_type)
    adv_feature_path = './detect_train/layers_features/{}_layers_{}.npy'.format(dataset_type,attack_type)
    base_feature_path = './detect_train/layers_features/{}_layers_baseline.npy'.format(dataset_type)
    train_feature_path = './detect_train/layers_features/{}_layers_train.npy'.format(dataset_type)
    ...
    feature_path = 'detect_train/{}_{}_{}.npy'.format(args.dataset, args.attack, args.type)
    print('base_features:',len(test_features[1][0]))
    ...
    if not os.path.isfile(feature_path):
        ...
        #         train_kde_feature, train_label = get_layer_feature(raw_model, train_loader, nb_layer, device,
        #                                                            rand=10000)
    ...
    test_attack_type = args.attack
    if attack_type != test_attack_type:
        path = './detect_train/{}_{}_{}.npy'.format(args.dataset, args.test_attack, args.type)
        ...
        path1 = './detect_train/{}_{}_{}.npy'.format(args.dataset,args.attack,args.type)
