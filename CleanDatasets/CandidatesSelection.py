...
from RawModels.Utils.dataset import get_cifar10_test_loader, get_mnist_test_loader, get_svhn_test_loader, \
    get_fsmnist_test_loader
...
def main(args):
    ...
    assert args.dataset == 'MNIST' or args.dataset == 'CIFAR10' or args.dataset == 'SVHN' or args.dataset == 'STL10' or args.dataset == 'FSMNIST' or args.dataset == 'IMAGENET10'
    if dataset == 'MNIST':
    ...
    if dataset == 'FSMNIST':
        raw_model = MNISTConvNet().to(device)
        raw_model.load(path=raw_model_location, device=device)
        test_loader = get_fsmnist_test_loader(dir_name=dataset_location,
                                              batch_size=1,
                                              shuffle=False)
    if dataset == 'SVHN':
        raw_model = SVHNConvNet().to(device)
        raw_model.load(path=raw_model_location, device=device)
        test_loader = get_svhn_test_loader(dir_name=dataset_location,
                                           batch_size=1,
                                           shuffle=False)
    if dataset == 'STL10':
        ...
        _,test_loader = loadData('../RawModels/IMAGENET10/dataset/',
                                              batch_size=1,
                                 shuffle=False)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Candidate Selection for Clean Data set')
    ...
    parser.add_argument('--seed',
                        type=int,
                        default=100,
                        help='the default random seed for numpy and torch')
    ...
    parser.add_argument(
        ...
        default=1000,
        help=
        'the total number of candidate samples that will be randomly selected')
