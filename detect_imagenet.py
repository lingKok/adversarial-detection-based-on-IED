...
def main():
    ...
    parse.add_argument('--type', default='cs', choices=['knn', 'lcs', 'ics','cs', 'kd', 'lid', 'iacs', 'ilacs'])
    ...
    adv_load = adv_loader(attack_type, dataset_type, 32)
    test_feature_path = './Feature/layers_features/{}_test.npy'.format(dataset_type)
    adv_feature_path = './Feature/layers_features/{}_{}.npy'.format(dataset_type,attack_type)
    base_feature_path = './Feature/layers_features/{}_base.npy'.format(dataset_type)
    if os.path.isfile(test_feature_path):
        test_features,test_labels = np.load(test_feature_path,allow_pickle=True)
    else:
        os.makedirs('./Feature/layers_features/',exist_ok=True)
    ...
    print('base_features:',len(base_features[0]))
    if not os.path.isfile(feature_path):
        if detect_type == 'lid':

            adv_score = get_score_by_lid(base_features, adv_features)
            test_score = get_score_by_lid(base_features, test_features)
        elif detect_type == 'kd':
            train_kde_path = 'Feature/layer_features/{}_train.npy'.format(dataset_type)
            test_kde_path = 'Feature/layer_features/{}_test.npy'.format(dataset_type)
            adv_kde_path = 'Feature/layer_features/{}_{}.npy'.format(dataset_type,attack_type)
            if os.path.isfile(train_kde_path):
                train = np.load(train_kde_path,allow_pickle=True).item()
                print(type(train))
                train_kde_feature, train_label = train['feature'],train['label']
            else:
                os.makedirs('./Feature/layer_features/',exist_ok=True)
                train_kde_feature ,train_label = get_layer_feature(raw_model,train_loader,nb_layer,device,rand = 10000)
                np.save(train_kde_path,{'feature':train_kde_feature,'label':train_label})
            if os.path.isfile(test_kde_path):
                test = np.load(test_kde_path,allow_pickle=True).item()
                test_kde_feature, test_label = test['feature'],test['label']
            else:
                test_kde_feature ,test_label = get_layer_feature(raw_model,test_loader,nb_layer,device,rand=0)
                np.save(test_kde_path,{'feature':test_kde_feature,'label':test_label})
            if os.path.isfile(adv_kde_path):
                adv = np.load(adv_kde_path,allow_pickle=True).item()
                adv_kde_feature, adv_label = adv['feature'],adv['label']
            else:
                adv_kde_feature ,adv_label = get_layer_feature(raw_model,adv_load,nb_layer,device,rand=0)
                np.save(adv_kde_path,{'feature':adv_kde_feature,'label':adv_label})
            ...
            test_score = get_score_by_LACS(test_feature_all, adv_feature_all)
            adv_score = get_score_by_LACS(test_feature_all, adv_feature_all)
        elif detect_type == 'acs':
            test_score = get_score_by_ACS(test_feature_all, adv_feature_all)
            adv_score = get_score_by_ACS(test_feature_all, adv_feature_all)
        elif detect_type == 'cs':
            test_score = get_score_by_CS(test_feature_all, adv_feature_all)
            adv_score = get_score_by_CS(test_feature_all, adv_feature_all)
        elif detect_type == 'lcs':
            test_score = get_score_by_LCS(test_feature_single, adv_feature_single)
            adv_score = get_score_by_LCS(test_feature_single, adv_feature_single)
            # np.save('./Feature/' + attack_type + '.npy', [test_score, adv_score])
    ...
    if attack_type != test_attack_type:
        path = './Feature/{}_{}_{}.npy'.format(args.dataset, args.test_attack, args.type)
        ...
        path1 = './Feature/{}_{}_{}.npy'.format(args.dataset,args.attack,args.type)
